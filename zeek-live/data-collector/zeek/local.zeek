...
redef digest_salt = "fishy_5m8o8Jt41n1";
...
redef JSONStreaming::extra_files=2;
redef JSONStreaming::rotation_interval=15mins;

# Clear old zeek logs
event zeek_init() &priority=1000
{
    when (local rm = Exec::run(Exec::Command($cmd="rm -f *.log"))) {
    print("Clearing leftover logs");
    }
    print("Clearing leftover logs");
}
...
event zeek_init()
{
    ...
    f$exclude=set("actions", "email_dest");
